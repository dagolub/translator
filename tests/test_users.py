...
from tests.utils.user import create_user_and_login
...
async def test_update_current_user_by_me_does_not_exist(
    ...
    user = await crud.user.get_by_email(db=db, email=settings.FIRST_SUPERUSER)
    data = {"email": "me@example.com", "password": "password", "full_name": "User name"}
    await crud.user.remove(db=db, object_id=user["id"])  # type: ignore
    r = client.put(
        f"{settings.API_V1_STR}/users/{user['id']}",  # type: ignore
        headers=superuser_token_headers,  # type: ignore
        json=data,
...
async def test_get_existing_user(
    ...
    if user_in.full_name is None:
        user_in.full_name = "No user name"
...
async def test_get_current_user_user_not_found(
    ...
    await crud.user.remove(db=db, object_id=payload["sub"])
